...
SELECT * FROM (SELECT * FROM MESSAGE ORDER BY MNUM DESC) WHERE ROWNUM <= 3 AND USERID LIKE '%' ORDER BY MNUM DESC;
SELECT * FROM (SELECT * FROM MESSAGE ORDER BY MNUM DESC) WHERE ROWNUM <= 3 AND USERID LIKE '%';
SELECT * FROM MESSAGE WHERE ROWNUM <= 3 AND USERID LIKE '%'ORDER BY MNUM DESC;
...
select * from message;
select * from reply;
UPDATE MESSAGE SET REPLYCNT=2 WHERE MNUM=5; 
